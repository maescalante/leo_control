...
def setObst(posicionObstacle):
    global  empezar, listObs
    if posiscionObstacle not in listObs:
        listObs.append(posicionObstacle)
...
def libre(xCas, yCas):# Si se encuentra un obstaculo en ella
    ...
    distRef=[]
    for i in listObs:
        a= math.sqrt ((i.linear.x - xCas) ** 2 + (listObs[i].linear.y - yCas) ** 2)

        dist.append(a)
        b=i.linear.z/2 + distanciaCarro
        distRef.append(b)
    ans=True
